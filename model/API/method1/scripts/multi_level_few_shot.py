...
def load_mapping(dewey_csv: str) -> Tuple[Dict[str, str], Dict[str, str], Dict[str, str]]:
...
def pick_candidates(codes: List[str], name_map: Dict[str, str]) -> Dict[str, str]:
    uniq = sorted({zfill_code(c) for c in codes})
    return {c: name_map.get(c, f"Unknown code {c}") for c in uniq}
...
def get_bucket_key(level: int, code: str) -> Optional[str]:
    code = zfill_code(code)
    if level == 1:
        return code[0]  # 0-9 buckets
    if level == 2:
        return code[0]  # 0-9 hundreds bucket
    if level == 3:
        return code[:2]  # e.g., 43*
    return None


def build_support_map(df: pd.DataFrame, code_col: str, level: int) -> Tuple[Dict[str, Dict[str, str]], List[int]]:
...
def create_prompt(
    ...
    names_list = list(candidates.values())
...
def build_candidates_for_code(level: int, code: str, maps: Tuple[Dict[str, str], Dict[str, str], Dict[str, str]]) -> Dict[str, str]:
    code = zfill_code(code)
...
def run_level(
    ...
    code_col = {1: "DDC-L1", 2: "DDC-L2", 3: "DDC-L3"}[level]
