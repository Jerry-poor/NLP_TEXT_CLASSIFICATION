...
def zero_shot_classify_dataset_with_confidence(
    ...
    def process_row(args):
        ...
        # print(f"\nProcessing sample {idx + 1}/{len(df)} (Thread)...")
        ...
        with ThreadPoolExecutor(max_workers=concurrency) as executor:
            ...
            for future in as_completed(future_to_idx):
                ...
                try:
                    res = future.result()
                    results.append(res)
                    print(f"[{processed_count}/{len(df)}] Finished sample {idx + 1}")
                except Exception as exc:
                    print(f"Sample {idx + 1} generated an exception: {exc}")
    else:
        for task in tasks:
            print(f"Processing sample {task[0] + 1}/{len(df)}:")
    ...
    if results and 'true_ddc_code' in results[0]:
