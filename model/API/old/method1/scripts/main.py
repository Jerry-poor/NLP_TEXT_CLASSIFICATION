...
class ClassificationManager:
    ...
    def run_zero_shot_classification(self, sample_size: int = 10, random_seed: int = 42,
                                    exclude_first_n: int = 5, sampled_data: Optional[pd.DataFrame] = None,
        ...
        results = zero_shot_classify_dataset_with_confidence(
            temp_input_file, output_file, sample_size=0, provider=self.provider, model=self.model, concurrency=concurrency
        )
    ...
    def run_one_shot_classification(self, sample_size: int = 10, random_seed: int = 42,
        ...
        results = one_shot_classify_dataset_with_confidence(
            ...
            provider=self.provider, model=self.model, concurrency=concurrency
        )
    ...
    def run_few_shot_classification(self, sample_size: int = 10, random_seed: int = 42, 
        ...
        results = few_shot_classify_dataset_with_confidence(
            ...
            support_examples=support_examples, provider=self.provider, model=self.model, concurrency=concurrency
        )
...
def main():
    ...
    default_input = os.path.abspath(os.path.join(SCRIPT_DIR, '..', '..', 'datasets', 'AG_news', 'AG_news_test_with_DDC_Hierarchy_mod_26Nov25_english_mapped.csv'))
    ...
    parser.add_argument('--concurrency', type=int, default=1,
                       help='Number of concurrent API requests (defaults to 1 for sequential)')
    ...
    try:
        if 'zero' in methods_to_run:
            zero_shot_result = manager.run_zero_shot_classification(
                args.sample_size, args.random_seed, args.exclude_first_n, shared_sample, concurrency=args.concurrency
            )
        ...
        if 'few' in methods_to_run:
            few_shot_result = manager.run_few_shot_classification(
                ...
                exclude_first_n=args.exclude_first_n, sampled_data=shared_sample, concurrency=args.concurrency
            )
