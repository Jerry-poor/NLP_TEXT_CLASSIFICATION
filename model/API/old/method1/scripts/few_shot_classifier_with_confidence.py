...
def few_shot_classify_dataset_with_confidence(
    ...
    # Allow concurrency alias to override max_workers if > 1
    if concurrency > 1:
    ...
    def classify_one(idx_row):
        ...
        for category, confidence in predicted_categories:
            ...
            for c, name in ddc_categories.items():
                if name.lower() == str(category).lower():
                    code = c
                    break
    ...
    print(f"Running with concurrency: max_workers={max_workers}")
    results = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(classify_one, item): item[0] for item in df_eval.iterrows()}
        for fut in as_completed(futures):
            res = fut.result()
            results.append(res)
            idx = res["index"]
            pred = res.get("predicted_categories") or []
            top_pred = pred[0][0] if pred else "N/A"
            print(f"[{idx + 1}/{len(df_eval)}] predicted {top_pred} for level1")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Few-shot Text Classification (with confidence)")
    parser.add_argument("--input", type=str, default="AG_news_test_with_DDC_Hierarchy_mod_26Nov25_english_mapped.csv")
    parser.add_argument("--output", type=str, default="few_shot_classification_with_confidence_results.csv")
    parser.add_argument("--sample_size", type=int, default=10, help="Set to 0 for all data")
    ...
    try:
        few_shot_classify_dataset_with_confidence(
            args.input,
            args.output,
